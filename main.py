...
def setup_llm_search(
    ...
    logger.info(f"-*- Creating {llm_id} LLM Search -*-")
    ...
    if ddg_search:
        tools.append(DuckDuckGo(fixed_max_results=20))
    if shell_tools:
    ...
    if research_assistant:
        _research_assistant = Assistant(
            ...
            tools=[ExaTools(num_results=100, text_length_limit=1000)],
            markdown=True,
    ...
    # 3. 4. Create LLM  Agent, Knowledge, Memory
    llm_search = Assistant(
        name="llm_search",
        llm=Ollama(model="llama3:8b"),
        description=dedent(
            """\
            You are the most advanced AI system in the world called `llm_search`.
            You have access to a set of tools and a team of AI Assistants at your disposal.
            Your goal is to assist the user in the best way possible to find a job.\
            """
        ),
        instructions=[
        ...
        storage=PgAssistantStorage(table_name="llm_search_runs", db_url=db_url),
        knowledge_base=AssistantKnowledge(
            vector_db=PgVector2(
                db_url=db_url,
                collection="llm_search_documents",
                embedder=OllamaEmbedder(model="mxbai-embed-large", dimensions=1536),
            ),
            num_documents=3,
        ),
        tools=tools,
